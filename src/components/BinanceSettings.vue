...
const orderStatus = value => ({ NEW: '未成交', PARTIALLY_FILLED: '部分成交', FILLED: '已成交', CANCELED: '已撤单', EXPIRED: '已过期', REJECTED: '已拒绝' }[value] || value || '—');
const dateTime = value => Number(value) > 0 ? new Date(Number(value)).toLocaleString() : '—';
...
function submitOrder() {
  ...
  const environment = binance.value.demo ? 'Demo 模拟盘' : '实盘';
  const detail = environment + ' · ' + symbol + ' · ' + orderForm.side + ' ' + orderForm.type
    + ' · 数量 ' + quantity
...
function closePosition(position) {
  ...
  const detail = (binance.value.demo ? 'Demo 模拟盘' : '实盘') + ' · ' + position.symbol
...
<template>
  <div class="trading-layout">
    <section class="ai-settings">
      ...
      <fieldset class="settings-group"><legend>账户连接</legend>
        <div class="environment-credentials">
          <article class="environment-card demo-card">
            <div class="environment-card-head"><div><b>Demo Trading</b><small>demo-fapi.binance.com · 模拟资金</small></div><label class="sync-toggle"><input v-model="trader.syncPaperOrdersToDemo" type="checkbox" /><span>同步模拟单</span></label></div>
          ...
          <article class="environment-card live-card">
            <div class="environment-card-head"><div><b>USDⓈ-M 实盘</b><small>fapi.binance.com · 真实资金</small></div><label class="sync-toggle"><input v-model="trader.syncPaperOrdersToLive" type="checkbox" /><span>同步模拟单</span></label></div>
      ...
      <fieldset class="settings-group control-group"><legend>账户与订单操作</legend>
        ...
        <div class="button-row control-actions">
          ...
          <button class="ghost" @click="$emit('refresh-positions', querySymbol.trim().toUpperCase() || undefined)" :disabled="loading">刷新持仓</button>
          <button class="ghost" @click="$emit('refresh-orders', querySymbol.trim().toUpperCase() || undefined)" :disabled="loading">刷新挂单</button>
        </div>
        ...
        <label class="wide-label">查询交易对（可选）<input v-model="querySymbol" placeholder="留空查询全部；例如 BTCUSDT" /></label>
        ...
        <div class="control-section-head"><h3>当前持仓 <span>{{ control.positions?.length || 0 }}</span></h3><small>模式：{{ control.positionMode === 'hedge' ? '双向' : '单向' }}</small></div>
        ...
        <p v-else class="control-empty">暂无持仓。点击“刷新持仓”读取 Binance 当前账户状态。</p>

        <div class="control-section-head"><h3>当前挂单 <span>{{ control.openOrders?.length || 0 }}</span></h3><small>普通 LIMIT / MARKET 订单</small></div>
        ...
        <p v-else class="control-empty">暂无挂单。点击“刷新挂单”读取 Binance 当前未成交订单。</p>
      </fieldset>
    </section>
...
.environment-card-head b { display: block; color: var(--text-primary); font: 700 13px var(--font-mono); }
