...
async function main() {
  ...
  const hr = await c.query(
    `SELECT extract(hour from entry_at) h, count(*) n, count(*) FILTER (WHERE net>0) wins,
      round(${wr},1) wr, round(avg(net)::numeric,3) avg_net
     FROM simulated_orders WHERE ${base} GROUP BY 1 ORDER BY 1`, [SINCE]);
  console.log('\n=== 按入场小时 ===');
  hr.rows.forEach(r => console.log(`${String(r.h).padStart(2,'0')}时: ${r.n} 单, 胜率 ${r.wr}%, 均单 ${r.avg_net}`));

  // ATR 相关：如果表里有 entry 相关字段，看看入场时的波动
  const cols = await c.query(`SELECT column_name FROM information_schema.columns WHERE table_name='simulated_orders' ORDER BY ordinal_position`);
  console.log('\n=== 表字段 ===');
  console.log(cols.rows.map(r => r.column_name).join(', '));

  // 最差币种（本批 >=5 单）
  const sym = await c.query(
    `SELECT symbol, count(*) n, count(*) FILTER (WHERE net>0) wins, round(${wr},1) wr,
      round(sum(net)::numeric,2) net, round(avg(net)::numeric,3) avg_net
     FROM simulated_orders WHERE ${base} GROUP BY symbol HAVING count(*)>=5 ORDER BY sum(net) ASC LIMIT 15`, [SINCE]);
  console.log('\n=== 本批最差币种(>=5单) ===');
  sym.rows.forEach(r => console.log(`${r.symbol}: ${r.n} 单, 胜率 ${r.wr}%, 净 ${r.net}, 均单 ${r.avg_net}`));
